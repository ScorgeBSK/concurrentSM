...
int speakerTick(){
	...
	switch(S_state){
		...
		case speaker_off:
			...
			break;
		case speaker_on:
			S_state = button ? speaker_off : speaker_init;
			break;
		default:
...
enum Freq_States {f_start, f_init, f_wait, increment, inc_release, decrement, dec_release} f_state;
void freqTick(){
	unsigned char upButton = ~PINA & 0x02;
	unsigned char downButton = ~PINA & 0x01;
	unsigned long tmpFreq = currentFreq;
	...
	switch(f_state){
		...
		case increment:
			...
			break;
		case inc_release:
			f_state = upButton ? inc_release : f_wait;
			break;
		case decrement:
			f_state = dec_release;
			break;
		case dec_release:
			f_state = downButton ? dec_release : f_wait;
			break;
		default:
			f_state = f_start;
			break;
